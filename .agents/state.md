...
- On `netwatch-01`, pull `main`, run `catchup`, and resolve the successful
  CI/Desktop runs whose `headSha` equals the pulled `origin/main`; the handoff
  push itself triggers that replacement candidate. Download its Windows
  installer and candidate metadata through the browser, require the manifest
  hash, then check SmartScreen behavior, normal per-user install, launch,
  visible About version `0.1.64`, `--native-policy-smoke`, and uninstall. Do
  not disable or bypass Defender or SmartScreen. Stop immediately and report
  the first failure. Once Windows passes, resume the same replacement
  candidate's remaining macOS/Linux, UI, Open Anyway, and freshly authorized
  Neon gates before requesting publication approval.
...
- The macOS Open Anyway test and Neon full write each require action-time
  authorization. The Windows host is available, but its remaining release
  checks require the local Windows Codex session so SmartScreen and About can
