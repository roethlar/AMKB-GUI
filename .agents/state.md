...
- AM Neon 80 support is in progress on `neon-80-support`. Plan tasks N1-N9 are
  implemented, and all ten `n567` review findings are repaired, red-proven, and
  verified but have not been re-reviewed. Nothing has ever been written to the
  keyboard; all device interaction so far has been read-only. The approved plan is
...
- Next action: perform manual, owner-present N10 hardware verification using
