...
- **OpenKeeb v2 planning:** capability-based support and firmware-resident
  lighting direction are settled. Next decide the optional QMK companion
  firmware boundary, then exact pilot boards and public technical identifiers
  one owner decision at a time. No implementation until the durable plan is
  approved.
