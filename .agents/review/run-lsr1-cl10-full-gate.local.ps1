...
function Write-Status {
    ...
    [System.IO.File]::WriteAllText($statusPath, ($Value | ConvertTo-Json -Depth 20), $utf8NoBom)
}
...
function Invoke-Step {
    ...
    if (-not $process.Start()) { throw "Failed to start gate step $($Step.Name)." }
    ...
    [System.IO.File]::AppendAllText($stdoutPath, "--- $($Step.Name) ---`r`n$stdout", $utf8NoBom)
    [System.IO.File]::AppendAllText($stderrPath, "--- $($Step.Name) ---`r`n$stderr", $utf8NoBom)
