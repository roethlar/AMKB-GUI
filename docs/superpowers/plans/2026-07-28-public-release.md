...
- the third replacement candidate from Desktop run `30416604544` (run 37) and
  ...
  supplied concrete MIME and extension tokens that WKWebView's native chooser
  did not enable reliably. The candidate is rejected.
...
- the fourth replacement candidate from Desktop run `30419761672` (run 38) and
  CI run `30419761680` at
  `f64aa974a047d680c8d04722d3f3fb66293eb79a` passed. Its candidate metadata
  regenerated byte-for-byte and all five attested subjects verified. The
  installer SHA-256 values are
  `35c4dbe052c277c8eefac55000438249d1c4f9a7f9dcb9a4613e55e65df70173`
  (macOS),
  `91c4745354eb40d166d9affdc3f6a2ef378d7dfcaa1d964c7cdd02d315407481`
  (Windows), and
  `b24400b20398f821e80236011552906e1ae62ac98c84c6555219d1447be19261`
  (Linux). Exact macOS native, signature, version, policy, UI, and GET-only
  Neon checks passed, as did the available Linux and unsigned-Windows package
  checks. The candidate is nevertheless rejected: its packaged HTML still
  carried `accept="image/*"`, and fresh valid GIF, PNG, and BMP files appeared
  in the native chooser with **Open** disabled. HTML file inputs are owned by
  WKWebView and do not use pywebview's Python `create_file_dialog` wildcard-UTI
  mapping.
- the replacement removes the media input's `accept` attribute entirely while
  retaining the authenticated 12 MB envelope, signature sniffing, complete
  GIF/PNG/BMP decoding, and fail-closed publication checks at the import
  endpoint. The focused browser guard failed against `image/*` and passes after
  removal. The clean repository gate passes 687 Python tests (one skipped), 83
  web tests, compilation/syntax checks, and source/wheel builds. A clean local
  DMG passes frozen smoke, the real WKWebView native-policy smoke,
  `hdiutil verify`, and deep strict ad-hoc signature verification; it reports
  `0.1.64`, CDHash `991ccb95cb09f3d53391dc7ed04a35046b8e36f8`, size
  25,226,045 bytes, and SHA-256
  `74090c617f37e1f0eddcea985f0587c120949b81275e9c23d15b1f56e634cc4b`.
  An exact app copy launched with isolated data and Library roots. The native
  chooser enabled GIF, PNG, and BMP, each rendered a Studio preview, and their
  Library assets matched the selected bytes at SHA-256
  `f5e96bff431bcdc2ece0ebbb25b1db8f72f54d57ae792db7e58a2ff9ea75967c`,
  `68837e574effe8da34a5d844d61ee2373dc6fd5ae19483a8d4ac63eb45b6c034`,
  and
  `2dca17ef87e5fc3ab0160d7379d534caa4a6fb81acff1ba06eee0cb2985ec49d`,
  respectively. A selected plain-text file reached the endpoint, was rejected
  as unsupported, and created no fourth Library item.
