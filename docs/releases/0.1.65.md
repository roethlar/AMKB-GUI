...
the [installation guide](../installing.md); do not disable operating-system
security globally.
...
- The Windows package used as the release prerequisite was installed, audited,
  smoke-tested, and uninstalled successfully. Final publication still requires
  the exact release candidate to pass every platform gate.
- The Neon device path has prior physical validation on one AM Neon 80, and
  publication is gated on a fresh check of the exact candidate. Other keyboard
  families have implementation, fixture, protocol, geometry, and regression
